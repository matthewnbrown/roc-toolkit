...
class ROCCaptchaSelector():
    ...
    __keypadTopLeft = { 'roc_recruit': [890, 705], 'roc_armory': [973,1011]}
    __keypadGap = [52, 42]
    ...
    def get_xy_static(self, number, page):
        if page not in self.__keypadTopLeft:
            raise Exception(f'Page {page} does not have coordinates for captchas!')
        number = int(number) - 1
        x_btn = self.__keypadTopLeft[page][0] + (number % 3) * self.__keypadGap[0]
        y_btn = self.__keypadTopLeft[page][1] + (number // 3) * self.__keypadGap[1]

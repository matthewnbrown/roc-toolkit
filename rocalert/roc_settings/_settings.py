...
def is_negative_string(val: str) -> bool:
...
def is_positive_string(val: str) -> bool:
    return val.lower().strip() in [
        "true",
        "yes",
        "1",
...
def time_conv(t: str):
    return (
        dt.strptime(t, "%H:%M").time()
        if len(t) <= 5
        else dt.strptime(t, "%H:%M:%S").time()
    )
    ...
    def load_settings_from_path(
        ...
        for line in lines:
            ...
            if setting_name not in default_settings:
                settings[setting_name] = Setting(setting_name, setting_name)
                if warnings:
                    print(
                        f"Warning: Setting {setting_name} found "
                        + "that is not in defaults"
                    )
            else:
                settings[setting_name] = copy.copy(default_settings[setting_name])
...
class SettingsValidator:
    def __check_dict_generic(
        setdic: dict[str, Setting], key: str, default: Setting, callback: Callable
    ) -> None:
        if key not in setdic:
            print(
                f"Warning: setting {key} not in settings."
    ...
    def validate_set(
        settings: dict, settings_to_validate, validationfunc: Callable
    ) -> bool:
    ...
    def check_settings_in_range(
        settings: dict[str, Setting], warnings: bool = False
    ) -> bool:
        valid = True
